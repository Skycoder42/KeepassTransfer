...
# build keepass transfer server
cd /tmp/src
qmake "PREFIX=/usr" "CONFIG+=no_clients" "CONFIG+=no_webapp"
make
make install

#create special symlinks, dirs and move the env script
mkdir -p /etc/Skycoder42
mv /tmp/src/server/dockerbuild/env_start.sh /usr/bin/

# test if working
/usr/bin/$KPT_NAME --version
